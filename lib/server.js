...
var serve = function(opts) {
  ...
  server.on('request', function(request) {
    ...
    sendFns[id] = util.send(ws, socketReplyEvents);

    util.log(ws.name + ' connected', opts);

    ws.on('message', util.receive(messageEvents, ws, socketReplyEvents));
    ...
    connectEvents.emit('connect', id, ws);
  });
  ...
  var send = function(clientId, type) {
    ...
    sendFns[clientId].apply(undefined, args);
  };
